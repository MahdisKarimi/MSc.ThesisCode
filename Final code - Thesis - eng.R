...
required_packages <- c("statmod", "mvtnorm", "corpcor", "ggplot2", "tidyr", 
                       "knitr", "survival", "viridis", "progress", "dplyr") # dplyr added
...
tryCatch({
  ...
  source("plot_CV.R"); source("make_filename.R") # make_filename and plot_CV functions are important
  # Phase 7
...
run_CV_cyclic_grid_search <- function(
    ...
    checkpoint_file = NULL # Optional: filename to save/load grid search state
) {
  ...
  if (nrow(grid_search_results) > 0 && sum(!is.na(grid_search_results$mean_cv_error)) > 0) {
    ...
    # Remove rows where mean_cv_error is NA
    ...
    if (nrow(valid_results) > 0) {
      ...
      } else if (metric == "deviance") { # Assumption: larger deviance is better
      ...
      return(list(
        ...
        all_grid_results = grid_search_results # return all results, including NAs for plotting
      ))
    } else {
...
run_full_scenario_analysis <- function(
  ...
  # This part can be moved outside the main function if the data is the same for multiple scenarios
  ...
  if (force_rerun_cv_cyclic || !file.exists(checkpoint_cv_cyclic_file)) {
    tryCatch({
      optimal_m_stops_cyclic_res <- run_CV_cyclic_grid_search(
        ...
        checkpoint_file = checkpoint_cv_cyclic_file # Pass filename for internal checkpointing
      )
  ...
  if (!is.null(optimal_m_stops_cyclic_res) && !is.null(optimal_m_stops_cyclic_res$best_m_stop_y0)) {
    ...
    if (generate_plots && !is.null(optimal_m_stops_cyclic_res$all_grid_results) && nrow(optimal_m_stops_cyclic_res$all_grid_results) > 0) {
      tryCatch({
        ...
        # Remove rows with NA error for plotting
        ...
        if(nrow(plot_data_cyclic_cv_valid) > 0){
          ...
          # Set fill direction for color scale (lower is better for loglik)
  ...
  # Save final models (Checkpointing)
  ...
  if (generate_tables) {
    ...
    # Helper function to dynamically create coefficient table
    ...
    # Create and save tables for Cyclic and Combined models
  ...
  if (generate_plots && length(all_brier_curves_test) > 0) { 
    tryCatch({
      ...
      # Convert model_type to a factor with specific order and labels
  ...
  if(generate_plots && !is.null(true_beta_train) && !is.null(true_gamma_train)){
    tryCatch({
      ...
      if(length(param_data_list) > 0) {
        ...
        # Build final dataframe with a 'source' column for precise coloring and legend
  ...
  if(generate_plots && !is.null(sim_data_test) && ( !is.null(model_cyclic_final) || !is.null(model_combined_final) ) ){
    tryCatch({
      km_test_fit <- survfit(Surv(times_test, delta_test) ~ 1) # requires survival package
      
      plot_data_surv <- data.frame(time = km_test_fit$time, km_survival = km_test_fit$surv)
      
      # For simplicity, calculate survival for an "average" individual (e.g., zero covariates except intercept)
      if(!is.null(model_cyclic_final)){
      ...
      p_surv_comp <- ggplot(plot_df_surv_long, aes(x=time, y=survival_probability , color=method, linetype=method)) +
        geom_step(linewidth=1) + # geom_step is better for Kaplan-Meier
...
# Scenario 3: High dimensions (huge), N_train=1000 (Note: time-consuming)
